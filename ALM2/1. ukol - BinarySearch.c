...
int BinarySearch(const char** A, int k, int l, char* x) {
	int s;
	if (k > l)
		return -1;
	s = (k + l) / 2;
...
int main() {
	...
	int i, bublani, pozice;
	char jmeeno[30];

	for (i = 1; i < Pocet; i++) {
		for (bublani = Pocet - 1; bublani >= i; bublani--) {
			if (strcmp(Jmena[bublani - 1], Jmena[bublani])>0) {

				t = Jmena[bublani - 1];
				Jmena[bublani - 1] = Jmena[bublani];
				Jmena[bublani] = t;
			}
		}
	}
	...
	pozice = BinarySearch(Jmena, 0, Pocet, jmeeno);

	if (pozice == -1) printf("jmeno %s nebylo nalezeno\n", jmeeno);
	else printf("pozice: %i  jmeno: %s", pozice, jmeeno);
